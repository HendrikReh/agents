  ...
  | `Assoc fields as json -> (
      ...
      | Some (`String "finish") ->
          let finish =
            {
              id = string_field fields "id";
              summary = opt_string_field fields "summary";
            }
          in
          Finish finish
      | Some (`String other) ->
          parse_errorf "Unsupported node type '%s'" other
      | Some _ -> parse_errorf "Node 'type' must be string"
      | None ->
          parse_errorf "Node missing 'type': %s" (Yojson.Safe.to_string json))
...
let pp_node fmt = function
  ...
  | Finish f ->
      Format.fprintf fmt "Finish(%s)" f.id
