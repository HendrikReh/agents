...
let default_model = "gpt-4"  (* gpt-5 doesn't exist yet *)
...
let chat ?(temperature = 0.2) ?(model = None) t ~messages =
  ...
  let body =
    `Assoc
      [
        "model", `String (Option.value ~default:t.default_model model);
  ...
  | Ok body_str -> (
      ...
      with Yojson.Json_error msg -> Lwt.return_error ("Failed to parse OpenAI JSON: " ^ msg)
    )
