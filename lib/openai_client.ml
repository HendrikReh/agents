...
let endpoint t path =
  ...
  let combined_path =
    if current_path = "" || current_path = "/" then path
    else Filename.concat current_path path
  in
  Uri.with_path base combined_path

let headers t =
  Cohttp.Header.of_list
    [ ("Authorization", "Bearer " ^ t.api_key); ("Content-Type", "application/json") ]
...
let chat ?(temperature = 0.2) ?(model = None) t ~messages =
  ...
  let body_fields =
    ...
    in
    if is_gpt5 then base else ("temperature", `Float temperature) :: base
  in
  if is_gpt5 then
    Log.info (fun m ->
        m "Sending chat request (model=%s, temperature=default, messages=%d)" model_name
          (List.length messages) )
  else
    Log.info (fun m ->
        m "Sending chat request (model=%s, temperature=%.2f, messages=%d)" model_name
          temperature (List.length messages) );
  let body = `Assoc body_fields |> Yojson.Safe.to_string in
  let body = Cohttp_lwt.Body.of_string body in
  Cohttp_lwt_unix.Client.post ~headers:(headers t) ~body endpoint >>= handle_response
  >>= function
  ...
  | Ok body_str -> (
    try
      ...
      | choice :: _ ->
        ...
        Log.debug (fun m ->
            m "Received response content (%d chars)" (String.length content) );
